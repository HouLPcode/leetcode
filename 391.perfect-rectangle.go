 ...
 */
//  164 ms, faster than 9.68%
// 8.8 MB, less than 7.69%
// 限制条件 ：
...
func isRectangleCover(rectangles [][]int) bool {
	...
	for _, v := range rectangles {
		sumArea += getArea(v)
		a := getAngles(v)
		numMap[fmt.Sprint(a[0])]++
		numMap[fmt.Sprint(a[1])]++
		numMap[fmt.Sprint(a[2])]++
		numMap[fmt.Sprint(a[3])]++
	...
	for k, v := range numMap {
		if v == 1 {
			cnt++
			if k != fmt.Sprint(minmaxAngles[0]) &&
				k != fmt.Sprint(minmaxAngles[1]) &&
				k != fmt.Sprint(minmaxAngles[2]) &&
				k != fmt.Sprint(minmaxAngles[3]) {
				return false
			}
		} else if v&1 != 0 {
			return false
